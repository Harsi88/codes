...
/*void play(){
	...
	for(i=1;i<9;++i){
		for(j=1;j<9;++j){
			...
			else if(board[i][j]=='K'){KING=1;}
		}
		if(king==1 && KING==1){break;}
	}
	}
	if(king==0){printf("PLAYER2 WON\n");}
	else if(KING==0){printf("PLAYER1 WON\n");}
	return;
}
...
main(){
	//play();
	int i1=0,i2,j1,j2,flag;
	chessboard();
//	swp(&board[2][8],&board[3][8]);
	printboard();
	while(i1<=8){
	flag=0;
	printf("enter move:");
	scanf("%d,%d,%d,%d",&i1,&j1,&i2,&j2);
	{if(movecheck(i1,j1,i2,j2)==1){flag=1;}
	else {printf("move not valid\n");}}
//	printf("%d\n",flag);
	if(flag==1){
		int i=movecheck1(i1,j1,i2,j2);
		if(i==1)printboard();
		else if (i==0)printf("Move not valid\n");
	}
	}
}
